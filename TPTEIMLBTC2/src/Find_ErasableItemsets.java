...
public class Find_ErasableItemsets
{
	...
	public Find_ErasableItemsets(char[][] dbData, int[] transProfit, double minGain,  double totalGain, ElementaryTable E1, ElementaryTable E2, int maxLength, int[] period, int[] itemStartPeriod, int[] periodtotalgain, int LSPall)
    {
	      try{			
				    //---------------------------
				    //宣告變數區
		    ...
		    try{							   
				    ...
				    //System.out.println(" 刪除前 - "+  EI[Current_Len-1].size()+" 個 candidate itemsets");
	       			...
	       			{      					
					    ...
					    {		
								if(period[m]<LSPall)	//只抓出LSPall之後的
									continue;//下一筆交易
