...
public class Find_E2 {
	...
	public Find_E2(char[][] dbData, int[] transProfit, double minGain, ElementaryTable E1, int [] period, int [] itemStartPeriod, int [] periodtotalgain)
	{
		try{
	...
	{		
		try{			
			...
			Set set0 = E1.ElementSet();
 			...
 			while (iter0.hasNext())   //E1_arr放所有E1的陣列
 			{
			...
			//C1 elementary table  將DB內出現過的所有item當作candidate
			for(int i=0;i<dbData.length;i++)
			{	for(int j=0;j<dbData[i].length;j++)
				{
					C1.add(dbData[i][j], 0);      //第i筆交易的第j個item
				}
			}
			
			
			//宣告C1_arr陣列  將C1放到C1_arr
			char[] C1_arr = new char[C1.size()];
			int position=0;
			Set set10 = C1.ElementSet();
 			Iterator iter10 = set10.iterator();
 			Element nx10;
 			
 			while (iter10.hasNext())   //C1_arr放所有C1的陣列
 			{
 	   		    nx10 = (Element)iter10.next();
				C1_arr[position] = nx10.getKey()[0];
				position++;
 			}
 			
 			// sort items in C1 (order by itemStartPeriod) 由大到小
			for(int i=0;i<C1_arr.length-1;i++)
			{
				for(int j=i+1;j<C1_arr.length;j++)
				{	int item1 = (int)C1_arr[i];
					int item2 = (int)C1_arr[j];
					if(itemStartPeriod[item1]<itemStartPeriod[item2])
					{
						char item = C1_arr[j];
						C1_arr[j] = C1_arr[i];
						C1_arr[i] = item;
			...
			//generate all possible candidate 2-itemsets from sorted C1_arr
			//排序目的: 確保前項的LSP會小於等於後項LSP
			for(int i=0;i<C1_arr.length-1;i++)
			{	if(E1.ContainsKey(C1_arr[i])) {
					for(int j=i+1;j<C1_arr.length;j++)
					{	int item1 = (int)C1_arr[i];
						int item2 = (int)C1_arr[j];
						if(itemStartPeriod[item1]==itemStartPeriod[item2] && E1.NoContainsKey(C1_arr[j]))
							continue; //Prune 1 若前後項LSP相同, 而後項不是 temporal erasable, 直接刪去
						else {
					...
					continue; //Prune 2 若前項不是temporal erasable, 直接刪去
			}
			...
			int itemsetStartPeriod;							//記錄此2-itemset的起始時間
 			...
 			{	itemsetStartPeriod=0;
				...
				{	
					if(period[a]<itemsetStartPeriod)	//只抓出此itemset的生命週期中的交易
						continue;//下一筆交易
