...
class Gen_C{
	...
	public Gen_C(char[][] C, ElementaryTable Preceding_LItemsets, int length, int[] itemStartPeriod) {  //建立的同時設定大小基準
      				  ...
      				  for(int j=i+1;j<C.length;j++)	
      				  {
       					  ...
       					  System.arraycopy(C[j],0,str2,0,str2.length);
       						...
       						if (CBC(str1,str2)){ //檢查除了最後一個item以外, 其餘items都要相同
	       						...
	       						if (Prune(temp,length, Preceding_LItemsets, itemStartPeriod)) {  ///修剪階段    							       							      							     						       									      										
	       							//對候選集做排序(順序統一由大到小) 
...
//==================== 修剪的方法 ====================	
	public static boolean Prune(char[] a,int length, ElementaryTable Preceding_LItemsets, int[] itemStartPeriod) //a欲修剪的itemset
	{		
			//記錄此candidate itemset的item start period
			int candidateSTP = 0;
			for (int k=0; k<a.length; k++) {
				int item = (int)a[k];
				if(itemStartPeriod[item]>candidateSTP)
					candidateSTP = itemStartPeriod[item];
				//System.out.print(item);  印出此 candidate itemset
			}
			//System.out.println("candidateSTP: " +candidateSTP);System.out.println(); //印出此candidate itemset的起始時間
			...
			for (int k=0; k<a.length; k++) {
       			...
       			//紀錄子項目集的item start period
       			int subitemsetSTP = 0;
    			for (int i=0; i<tmp.length; i++) {
    				int item = (int)tmp[i];
    				if(itemStartPeriod[item]>subitemsetSTP)
    					subitemsetSTP = itemStartPeriod[item];
    				//System.out.print(item);  //印出此 candidate itemset
    			}
    			//System.out.println();
    			//System.out.println("subitemsetSTP: " + subitemsetSTP); //印出此candidate itemset的起始時間
    			...
    			for(int i = 0;i<tmp.length;i++) {
       			...
       			//檢查
    			if(subitemsetSTP!=candidateSTP) continue;  		//若candidate itemset和subitemset的STP不相同, 直接不考慮
	       		if(Preceding_LItemsets.NoContainsKey(tmp)) {    //若相同, 看此sub-itemset是否為temporal erasable
