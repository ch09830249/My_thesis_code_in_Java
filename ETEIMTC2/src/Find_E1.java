...
public class Find_E1
{
	...
	public Find_E1(char[][] dbData, int[] transProfit, double minGain, int [] period, int [] itemStartPeriod, int [] periodtotalgain)
	{
		try
		{	
			//------------------------------------------
    ...
    public void Find_E1()
    {
		try
		{
			...
			while (iter1.hasNext())    
			{
				...
				//算其temporal gain ratio
				this.totalGain=0;
				for(int i=itemperiod;i<=periodtotalgain.length-1;i++) {	//加總此item出現期間其totalgain
